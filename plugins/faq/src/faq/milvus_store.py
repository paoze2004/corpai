...
- 首次 search/upsert lazy-connect(host/port from CorpAI.config.Config)
...
def _ensure_connected() -> MilvusClient:
    ...
    with _lock:
        ...
        if not client.has_collection(col_name):
            ...
            schema = CollectionSchema(fields, description="CorpAI faq KB")
...
def get_count() -> int:
    ...
    try:
        client = _ensure_connected()
        return int(client.get_collection_stats(_cfg.faq_collection).get("row_count", 0))
    except Exception:
        return 0
