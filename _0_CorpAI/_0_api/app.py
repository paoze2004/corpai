"""
需求：_0_CorpAI FastAPI后端服务器，提供REST API接口
"""
import json
import logging
import os
...
from _0_CorpAI._0_api import admin_router as _admin
from _0_CorpAI._0_api import demo_router as _demo  # M5
...
from _0_CorpAI._2_platform.plugin_manager import discover_all
...
try:
    ...
    from _0_CorpAI._2_platform.sre.webhook import router as sre_webhook_router
    _SRE_WEBHOOK_AVAILABLE = True
except ImportError:
    sre_webhook_router = None  # type: ignore[assignment]
    _SRE_WEBHOOK_AVAILABLE = False
    logger.warning("_0_CorpAI._2_platform.sre.webhook 未加载,飞书 webhook 路由降级")
from _0_CorpAI._2_platform.wiring import build_default_service
...
app = FastAPI(title="_0_CorpAI API", description="企业 AI Copilot 平台 — 可插拔多 Agent")
app.add_middleware(TraceContextMiddleware)

# OpenTelemetry —— 跟现有 trace.py 并行(默认关闭,设 OTEL_EXPORTER_OTLP_ENDPOINT 启用)
# FastAPIInstrumentor 给每个 HTTP 请求加一个 OTel span;现有 TraceContextMiddleware + call_records 不动
from _0_CorpAI._2_platform.observability.otel import init_otel  # noqa: E402
init_otel(app)
...
# Phase 3:include admin router + mount /admin _5_static + plugin_manager 注入到 build_default_service
app.include_router(_admin.router)
app.include_router(_demo.router)  # M5:/demo/trigger, /demo/stream/{run_id}, /demo/runs
_static_root = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "_5_static")
...
async def index():
    """返回前端页面"""
    # 静态资源在 _0_CorpAI/_5_static/，不是 _0_CorpAI/api/_5_static/
    # __file__ = _0_CorpAI/api/app.py → dirname 是 api/，需要再上一层
    static_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "_5_static")
...
def _setup_request_auth(authorization: str | None) -> None:
    """从 Authorization header 解析 JWT,设置 wiring 的 user_scopes + user_token。

    - 合法 JWT → set_user_scopes(claims.scopes) + set_user_token(raw token)
    - 非法 JWT  → 空 scopes + None token
    - 无 token  + DEV_NO_AUTH=1 → ["*"] + None token
    - 无 token  → 空 scopes + None token

    设计动机:让 /api/chat 和 /api/chat/stream 共享同一套 RBAC + A2A 透传逻辑。
    """
    from _0_CorpAI._2_platform.wiring import set_user_scopes, set_user_token
    if authorization and authorization.startswith("Bearer "):
        ...
        if claims:
            ...
            # A2A 透传:raw token 喂给 wiring,plugin server 从
            # task.metadata["authorization"] 拿出来当 Bearer header 用
            set_user_token(authorization[7:])
        else:
            set_user_scopes([])
            set_user_token(None)
    elif os.getenv("DEV_NO_AUTH", "").strip() == "1":
        set_user_scopes(["*"])
        set_user_token(None)  # plugin 端走 dev_token fallback
    else:
        set_user_scopes([])
        set_user_token(None)
...
async def chat(
    ...
    """
    _setup_request_auth(authorization)
