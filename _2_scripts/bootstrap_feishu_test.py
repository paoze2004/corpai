...
用法:
    ...
    uv run python _2_scripts/bootstrap_feishu_test.py --create-plan
    ...
    # 只看卡片 JSON,不真发
    uv run python _2_scripts/bootstrap_feishu_test.py --dry-run

    # 不写 DB(只测发卡通道,按钮回调进来 plan 不存在会 404)
    uv run python _2_scripts/bootstrap_feishu_test.py \\
...
# v3.2:统一从 _0_CorpAI._3_utils.dotenv 加载 .env(单一配置源)
# 顺便把项目根加进 sys.path(允许 `from _0_CorpAI.xxx import yyy`)
project_root = Path(__file__).resolve().parent.parent
if str(project_root) not in sys.path:
    sys.path.insert(0, str(project_root))
# 也加 _1_plugins/sre_copilot/src 让 sre_copilot.feishu 能 import(没装 pip 包时)
sre_plugin_src = project_root / "_1_plugins" / "sre_copilot" / "src"
...
def _create_pending_plan(
    ...
    from _0_CorpAI._2_platform.sre.approval import ApprovalService
