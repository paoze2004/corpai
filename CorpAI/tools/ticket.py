...
from CorpAI.utils.format import DateEncoder, default_encoder
...
class TicketService:
    ...
    def query_concert(self, city: str = None, artist: str = None, date: str = None, ticket_type: str = None) -> str:
        """
        查询演唱会/演出票。

        参数:
            city:        演出城市，如 "北京"（可选,但建议传）
            artist:      艺人名称，如 "周杰伦"（必填,缺则返回 missing_params 提示）
            date:        演出日期，格式 "YYYY-MM-DD"（必填,缺则返回 missing_params 提示）
            ticket_type: 票类型（可选），如 "VIP"、"普通票"

        返回:
            JSON 字符串，包含艺人、场馆、时间、票价、余票等信息；
            缺 artist/date 时返回 {"status":"missing_params",...} 让 LLM 追问用户。
        """
        # 业务必传参数缺失,返回友好提示(让 agent 知道要追问)
        if not artist or not date:
            return json.dumps(
                {"status": "missing_params",
                 "message": "查询演唱会需要艺人名称和演出日期,请向用户追问这两个信息。",
        ...
        sql = ("SELECT id, artist, city, venue, start_time, end_time, "
               ...
               "WHERE artist = %s AND DATE(start_time) = %s")
        params = [artist, date]
        if city:
            sql += " AND city = %s"
            params.append(city)
...
def create_ticket_mcp_server():
    ...
    @ticket_mcp.tool(
        name="query_concert",
        description="查询演唱会票信息,参数:city(城市,可选), artist(艺人,**必填**), date(演出日期YYYY-MM-DD,**必填**), ticket_type(票类型,可选)"
    )
    def query_concert(city: str = None, artist: str = None, date: str = None, ticket_type: str = None) -> str:
