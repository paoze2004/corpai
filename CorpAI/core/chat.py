...
# 导入项目内部模块（统一以 CorpAI. 包名前缀，便于任意入口运行）
from CorpAI.config import Config  # 配置模块（模型地址、API Key、意图映射等）
from CorpAI.logging import logger  # 日志模块，用于记录运行日志
from CorpAI.core.prompts import CorpAIPrompts  # Prompt 模板管理
from CorpAI.core.memory import ConversationMemory  # 记忆管理（短期记忆、用户偏好、任务上下文）
...
class ChatService:
    ...
    def __init__(self):
        ...
        3. 初始化记忆管理器
        4. 初始化对话历史和工具列表缓存
        5. 构建 OrchestratorService(Phase 1.7)— chat/chat_stream 委托给它
        """
        # ========== 1. 配置 A2A 代理网络 ==========
        # 记录每个子代理的访问地址
        self.agent_urls = {
...
""")
        ...
        # ========== 5. 构建 OrchestratorService(Phase 1.7) ==========
        # chat/chat_stream 方法委托给它,不再直接调 A2A / LLM
        # 5 个新模块(intent/planner/react_loop/streaming)由它管理
        self._orchestrator = self._build_orchestrator()

    def _build_orchestrator(self):
        """构造 OrchestratorService — DI 注入 6 个依赖。

        关键设计:
        - simple_step_executor: 委托给 self._call_agent_intent(非流式 A2A 调用)
        - attraction_executor: 委托给 self._run_attraction_pipeline(查天气 + LLM 生成)
        - messages_provider: 闭包返回 self.messages(让新模块能访问最新对话)
        """
        from CorpAI.platform.orchestrator import (
            IntentRecognizer, TaskPlanner, ReActRunner, OrchestratorService,
        )

        # simple step executor:走 A2A + summary chain
        async def simple_step_executor(intent: str, query_str: str) -> str:
            return await self._call_agent_intent(intent, query_str)

        # attraction executor:查天气 + LLM 生成推荐
        async def attraction_executor(query_str: str) -> str:
            return await self._run_attraction_pipeline(query_str)

        # messages provider:让 TaskPlanner/ReActRunner 能访问最近对话
        def messages_provider() -> list:
            return self.messages

        return OrchestratorService(
            intent=IntentRecognizer(llm=self.llm, memory=self.memory),
            planner=TaskPlanner(llm=self.llm, memory=self.memory, messages_provider=messages_provider),
    ...
    async def _run_attraction_pipeline(self, query_str: str) -> str:
        """attraction 完整流程:查目的地天气 + 调 LLM 生成推荐(原 chat.py:548-552 逻辑)。"""
        weather_info = await self._fetch_weather_for_attraction(query_str)
        chain = CorpAIPrompts.attraction_prompt() | self.llm
        chunks: list[str] = []
    ...
    async def _fetch_weather_for_attraction(self, query_str: str) -> str:
        ...
        try:
            ...
            raw_response = await asyncio.to_thread(asyncio.run, agent.send_task_async(task))
    ...
    def intent_agent(self, user_input: str):
        ...
        chain = CorpAIPrompts.intent_prompt() | self.llm
        ...
        # 清理 LLM 返回的"非 JSON"噪音，按顺序处理：
        # 1) Qwen3 / DeepSeek 等模型会在 JSON 前面输出 <think>...</think> 思维链
        # 2) 部分模型还会把 JSON 包在 ```json ... ``` 代码块里
        # 3) 兜底：直接从文本里抠出第一个 {...} 子串
        ...
        # 兜底提取：万一模型还夹杂了别的文字，取第一个完整的 JSON 对象
        ...
        try:
            ...
            # 兜底：LLM 有时被对话历史里的错误消息带偏，直接输出非 JSON 的回复
            # 此时把整段回复当 follow_up_message 返回给用户，避免硬报错
    ...
    def planning_agent(self, intents: list, user_queries: dict) -> dict:
        ...
        """
        chain = CorpAIPrompts.planning_prompt() | self.llm
        ...
        # 同 intent_agent：先剥 <think> 块，再剥 markdown 包裹，最后兜底提取 JSON
    ...
    async def _call_agent_intent(self, intent: str, query_str: str) -> str:
        ...
        if intent == "attraction":
            chain = CorpAIPrompts.attraction_prompt() | self.llm
            weather_info = await self._fetch_weather_for_attraction(query_str)
            return strip_think(chain.invoke({"query": query_str, "weather_info": weather_info}).content.strip())
        ...
        # ========== 情况2：需要调用子代理 ===========
            ...
            # 通过 A2A 协议异步发送任务给子代理
            # 用 asyncio.to_thread 把同步的 send_task_async 放到线程池跑，避免阻塞事件循环
            try:
                logger.info(f"调用 {agent_name}...")
                # send_task_async 返回 coroutine，要 asyncio.run 跑完才能拿到响应对象
                raw_response = await asyncio.to_thread(asyncio.run, agent.send_task_async(task))
                logger.info(f"{agent_name} 收到响应: {raw_response.status.state}")
            ...
            if agent_name == "WeatherQueryAssistant":
                chain = CorpAIPrompts.summarize_weather_prompt() | self.llm
                return strip_think(chain.invoke({"query": query_str, "raw_response": agent_result}).content.strip())
            elif agent_name == "TicketAssistant":
                chain = CorpAIPrompts.summarize_ticket_prompt() | self.llm
                return strip_think(chain.invoke({"query": query_str, "raw_response": agent_result}).content.strip())
            elif agent_name == "TripAssistant":
                chain = CorpAIPrompts.summarize_trip_prompt() | self.llm
                return strip_think(chain.invoke({"query": query_str, "raw_response": agent_result}).content.strip())
            else:
                # 其他代理直接返回原始结果
                return agent_result
    ...
    async def react_loop(self, steps: list, user_queries: dict) -> str:
        ...
        if len(step_results) > 1:
            ...
            summary_chain = CorpAIPrompts.react_summary_prompt() | self.llm
    ...
    async def chat(self, user_input: str) -> str:
        ...
        """
        # Phase 1.7: 委托给 OrchestratorService
        # OrchestratorService 内部完成所有流程并通过 memory.add_message 写入记忆
        # 然后我们同步更新 ChatService 自己的 messages / conversation_history(向后兼容)
        # (注意:不要在调用 orchestrator 之前 add_message,否则会重复记录)
        response = await self._orchestrator.chat(user_input)
    ...
    async def chat_stream(self, user_input: str):
        """
        流式处理用户输入的主方法 (Phase 1.7 委托给 OrchestratorService)

        Phase 1.6 简化版:yield 完整结果(非 token-level streaming)
        真正的 token streaming 留到 Phase 4+(ADR-008)
        ...
        """
        # Phase 1.7:委托给 OrchestratorService
        # OrchestratorService 内部完成所有流程,逐 chunk yield
        parts: list[str] = []
        async for chunk in self._orchestrator.chat_stream(user_input):
            parts.append(chunk)
            yield chunk
        # 同步更新 ChatService 自己的 messages / conversation_history
        response = "".join(parts)
        self.messages.append({"role": "user", "content": user_input})
        self.conversation_history += "\nUser: " + user_input
        self.conversation_history += "\nAssistant: " + response
    ...
    async def _call_agent_intent_stream(self, intent: str, query_str: str):
        ...
        if intent == "attraction":
            chain = CorpAIPrompts.attraction_prompt() | self.llm
            # 同步非流式版本会查天气，流式版本原来漏了 → 报"missing weather_info"
            weather_info = await self._fetch_weather_for_attraction(query_str)
            async for chunk in chain.astream({"query": query_str, "weather_info": weather_info}):
                yield strip_think(chunk.content) if hasattr(chunk, 'content') else str(chunk)
            ...
            if agent is None:
                ...
                try:
                    # send_task_async 返回 coroutine，需要 asyncio.run 在新事件循环里跑完
                    # 用 to_thread 包装避免阻塞主事件循环
                    logger.info(f"调用 {agent_name}...")
                    raw_response = await asyncio.to_thread(asyncio.run, agent.send_task_async(task))
                    logger.info(f"{agent_name} 收到响应: {raw_response.status.state}")
