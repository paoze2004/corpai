...
新增业务 plugin 的最小流程:
    1. 写 plugins/<name>/plugin.py,实现 `register(registry) -> None` 函数
    2. 在 plugins/<name>/pyproject.toml 的
       [project.entry-points."platform.plugins"] 段声明
       `name = "module.path:register"`
    3. `uv pip install -e ./plugins/<name>` 即可被 `discover_all()` 找到
"""
...
def discover_all() -> PluginRegistry:
    """扫描 entry_points group="platform.plugins" 自动加载。
