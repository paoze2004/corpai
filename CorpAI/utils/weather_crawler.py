...
from CorpAI.config import Config, env
...
def fetch_weather_data(city, location):
    """
    从和风天气 API 拉取指定城市的天气数据(JSON dict)。

    URL 结构：{api_host}{base_url}?location={location_id}
    Header：X-QW-Api-Key
    注意：不要手动设 Accept-Encoding，让 requests 自动协商压缩和解压，
         避免服务器响应头撒谎（声明 gzip 但实际是裸 JSON）时手动解压报错。

    :param city: 城市名（仅用于日志打印）
    :param location: 城市代码（如 101010100）
    :return: 天气数据 dict；失败返回 None
    """
    url = f"{cfg.weather_api_host}{cfg.weather_base_url}?location={location}"
    headers = {"X-QW-Api-Key": cfg.weather_api_key}
    try:
        ...
        # response.json() 在响应不是合法 JSON 时会抛 ValueError
...
def get_latest_update_time(cursor, city):
    ...
    fetchone() 返回单个元组，元素顺序与 SELECT 列顺序一致：
        ...
        # result => (datetime.datetime(2026, 4, 28, 10, 30, 0))
        # result[0] => "北京"
    ...
    # result[0] : datetime.datetime(2026, 4, 28, 10, 30, 0)
    # 若无数据 => (None,)
