...
读 sql/migrate_devops_scope_to_sre.sql 执行 UPDATE,加预览 + 影响行数。

跑法:`uv run python scripts/migrate_devops_scope_to_sre.py`(--或 `--dry-run` 看预览)
...
def main() -> int:
    ...
    ddl_path = (
        ...
        / "sql" / "migrate_devops_scope_to_sre.sql"
    )
