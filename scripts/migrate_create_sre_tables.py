"""跑 sql/create_sre_tables.sql。

用法:
    uv run python scripts/migrate_create_sre_tables.py
    uv run python scripts/migrate_create_sre_tables.py --dry-run   # 只看 SQL,不执行
...
SQL_FILE = ROOT / "sql" / "create_sre_tables.sql"
...
def main() -> int:
    ...
    database = __import__("os").environ.get("MYSQL_DATABASE", "CorpAI")
