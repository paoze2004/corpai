...
用法:
  PYTHONIOENCODING=utf-8 PYTHONPATH=. uv run python scripts/list_feishu_chats.py
...
依赖:
  ...
  - bot 必须加入过群(在飞书群里"添加机器人"把 CorpAI 运维机器人拉进去)
...
def main() -> int:
    ...
    if not chats:
        ...
        print("  → 在飞书里随便建一个群,把 CorpAI 运维机器人拉进去,再跑一次")
        return 3
    ...
    print("    uv run python scripts/bootstrap_feishu_test.py \\")
    print("    --create-plan \\")
