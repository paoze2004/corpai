...
from _0_CorpAI._2_platform.observability.metrics import HR_ACTION_TOTAL, HR_BRIDGE_ERRORS_TOTAL
from _0_CorpAI._2_platform.observability.trace import current_trace_id, new_trace_id
...
_FAQ_URL = os.getenv("KNOWLEDGE_URL", "http://localhost:8030")
...
def cross_query_knowledge(authorization: str, query: str, top_k: int = 2) -> str:
    """跨插件:调 faq mcp 拿语义检索结果(兜底补全)。

    适用:hr KB 不覆盖时,自动用 faq 共同回答。
    失败:v3.0 显式返 bridge_unavailable(不再 silent-fail)。
    """
    action = "cross_query_knowledge"
    try:
        _current_user(authorization)  # 校验起码 chat:write
        result = _bridge_call("knowledge", _FAQ_URL, "query_knowledge",
        ...
        HR_BRIDGE_ERRORS_TOTAL.labels(target="knowledge", kind="error").inc()
        return _err(action, "error", str(e))
