...
【跨插件 bridge 3 个】
- cross_query_knowledge — HR 业务未命中时兜底调 faq mcp 补全(只用于常见 FAQ)
...
3. 业务未命中常见 FAQ → 调 cross_query_knowledge 兜底

注意:本插件**不**回答通用 HR 政策/福利咨询(那是 faq plugin 的职责)。如用户问"年假怎么算"等政策类问题,直接调 cross_query_knowledge 兜底查询。
