...
try:
    ...
    _DEFAULT_COLLECTION = Config().knowledge_collection if hasattr(Config, "knowledge_collection") else "knowledge_docs"
except Exception:
    _DEFAULT_COLLECTION = os.getenv("FAQ_COLLECTION", "knowledge_docs")
...
def query_knowledge(query_text: str, collection: str | None = None, limit: int = 3) -> str:
    ...
    失败处理(用户决定硬失败):
    - Milvus 不可达 → raise MilvusUnreachable,KnowledgeServer 捕获后返 TaskState.FAILED
...
def query_faq_inmemory_fallback(query_text: str, collection: str | None = None, limit: int = 3) -> str:
    ...
    与 query_knowledge 同合同(JSON envelope)。
    """
    from knowledge import milvus_store as ms
