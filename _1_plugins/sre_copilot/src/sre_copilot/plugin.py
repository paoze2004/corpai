...
- _0_CorpAI/platform/sre/ 里 action_executor / action_tools / incident_manager / feishu / executor_cli 5 个文件搬进本 plugin
...
BRIDGE_KNOWLEDGE_TOOL = PluginManifest(
    name="sre_copilot_bridge_knowledge_mcp",
    version="3.1.0",
    description="SRE → FAQ 兜底:cross_query_knowledge(SOP 兜底)。失败显式告知 + Counter。",
    plugin_type="mcp_tool",
...
def register(registry: PluginRegistry) -> None:
    ...
    for m in (
        ...
        BRIDGE_HR_TOOL, BRIDGE_KNOWLEDGE_TOOL,
    ):
        registry.register(m)